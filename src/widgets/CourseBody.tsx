    ...
    return (
        <div className="">
            <div className="flex justify-between lg:justify-evenly items-center py-2 border-t border-b space-x-5 text-center">
                <strong className="font-bold cursor-pointer" onClick={() => setCurrentIndex(0)}>
                    Details
                </strong>
                <strong className="font-bold cursor-pointer" onClick={() => setCurrentIndex(1)}>
                    Curriculum
                </strong>
            </div>

            {currentIndex === 0 && (
                <div className="mt-5" dangerouslySetInnerHTML={{ __html: course?.content }} />
            )}
            {currentIndex === 1 && (
